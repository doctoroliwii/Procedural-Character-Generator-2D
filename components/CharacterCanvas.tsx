...
const CharacterCanvas: React.FC<CharacterCanvasProps> = ({ characters, backgroundOptions, showBoundingBoxes }) => {
  ...
  const eyeClipPathData = sortedCharacters.map((charInstance) => {
    const { params } = charInstance;
  ...
  return (
    ...
    >
      <defs>
        ...
        {eyeClipPathData.map((data, index) => (
            <React.Fragment key={`defs-${index}`}>
                <clipPath id={`leftEyeClip-${index}`}><path d={data.leftEyeClipPathD} /></clipPath>
                <clipPath id={`rightEyeClip-${index}`}><path d={data.rightEyeClipPathD} /></clipPath>
            </React.Fragment>
        ))}
      ...
      {sortedCharacters.map((charInstance, index) => {
        ...
        const localCursorX = (cursorPos.x - (VIEWBOX_WIDTH / 2 + charInstance.x)) / charInstance.scale + (VIEWBOX_WIDTH / 2);
        const localCursorY = (cursorPos.y - (VIEWBOX_HEIGHT / 2 + charInstance.y)) / charInstance.scale + (VIEWBOX_HEIGHT / 2);
        ...
        const eyelidCompensationY = (eyeRy * (upperEyelidCoverage / 100)) * 0.4; const glintRadius = calculatedEyeSize * 0.2; const glintOffsetX = eyeRx * 0.3; const glintOffsetY = eyeRy * 0.3; const finalEyebrowYOffset = Math.max(calculatedEyebrowYOffset, eyeRy + calculatedEyebrowHeight / 2 + 5); let eyebrowY = eyeYPos - finalEyebrowYOffset; const eyebrowTopPoint = eyebrowY - (calculatedEyebrowHeight / 2); const headTopLimit = actualHeadTopY + (outlineWidth / 2); if (eyebrowTopPoint < headTopLimit) { eyebrowY = headTopLimit + (calculatedEyebrowHeight / 2); }
        ...
        const getLocalBBox = () => {
            ...
            const addPoint = (p: {x:number, y:number}, marginX=0, marginY=0) => {
                minX = Math.min(minX, p.x - marginX); minY = Math.min(minY, p.y - marginY);
                maxX = Math.max(maxX, p.x + marginX); maxY = Math.max(maxY, p.y + marginY);
            };
            
            // Head and Hair
            let headTop = actualHeadTopY;
            if (hair) headTop = Math.min(headTop, hairAnchorY);
            let headLeft = centerX - headWidth/2;
            let headRight = centerX + headWidth/2;
            if (hair) {
                headLeft = Math.min(headLeft, centerX - backHairRx);
                headRight = Math.max(headRight, centerX + backHairRx);
            }
            addPoint({x: headLeft, y: headTop});
            ...
            addPoint(rArmControl, rArmWidth / 2, rArmWidth / 2);
        ...
        return (
            <g key={`char-${index}`} transform={`translate(${charInstance.x + VIEWBOX_WIDTH/2}, ${charInstance.y + VIEWBOX_HEIGHT/2}) scale(${charInstance.scale}) translate(${-VIEWBOX_WIDTH/2}, ${-VIEWBOX_HEIGHT/2})`}>
                ...
                <g>
                  <g><g clipPath={`url(#leftEyeClip-${index})`}><ellipse cx={leftEyeX} cy={eyeYPos} rx={eyeRx} ry={eyeRy} fill="white" /><ellipse cx={leftEyeX + leftPupilOffset.x} cy={eyeYPos + leftPupilOffset.y - eyelidCompensationY} rx={irisRx} ry={irisRy} fill={irisColor} /><ellipse cx={leftEyeX + leftPupilOffset.x} cy={eyeYPos + leftPupilOffset.y - eyelidCompensationY} rx={pupilRx} ry={pupilRy} fill={pupilColor} /><circle cx={leftEyeX + glintOffsetX + leftPupilOffset.x * 0.5} cy={eyeYPos - glintOffsetY + leftPupilOffset.y * 0.5 - eyelidCompensationY} r={glintRadius} fill="white" /></g>{eyeOutlines && <path d={eyeClipPathData[index].leftEyeClipPathD} fill="none" stroke={outlineColor} strokeWidth={outlineWidth / 2} strokeLinejoin="round" />}</g>
                  <g><g clipPath={`url(#rightEyeClip-${index})`}><ellipse cx={rightEyeX} cy={eyeYPos} rx={eyeRx} ry={eyeRy} fill="white" /><ellipse cx={rightEyeX + rightPupilOffset.x} cy={eyeYPos + rightPupilOffset.y - eyelidCompensationY} rx={irisRx} ry={irisRy} fill={irisColor} /><ellipse cx={rightEyeX + rightPupilOffset.x} cy={eyeYPos + rightPupilOffset.y - eyelidCompensationY} rx={pupilRx} ry={pupilRy} fill={pupilColor} /><circle cx={rightEyeX + glintOffsetX + rightPupilOffset.x * 0.5} cy={eyeYPos - glintOffsetY + rightPupilOffset.y * 0.5 - eyelidCompensationY} r={glintRadius} fill="white" /></g>{eyeOutlines && <path d={eyeClipPathData[index].rightEyeClipPathD} fill="none" stroke={outlineColor} strokeWidth={outlineWidth / 2} strokeLinejoin="round" />}</g>
                </g>
                {renderEyelashes(leftEyeX)} {renderEyelashes(rightEyeX)}
                {hair && fringePath && (<g><path d={fringePath} fill={hairColor} />{bodyOutlines && (<path d={fringeBottomOutlinePath} stroke={outlineColor} strokeWidth={outlineWidth} fill="none" strokeLinecap="round" />)}</g>)}
                <g transform={`translate(${leftEyeX}, ${eyebrowY}) rotate(${eyebrowAngle})`}><rect x={-calculatedEyebrowWidth / 2} y={-calculatedEyebrowHeight / 2} width={calculatedEyebrowWidth} height={calculatedEyebrowHeight} fill={pupilColor} rx={2} /></g>
                <g transform={`translate(${rightEyeX}, ${eyebrowY}) rotate(${-eyebrowAngle})`}><rect x={-calculatedEyebrowWidth / 2} y={-calculatedEyebrowHeight / 2} width={calculatedEyebrowWidth} height={calculatedEyebrowHeight} fill={pupilColor} rx={2} /></g>
                <path d={mouthPath} stroke={pupilColor} strokeWidth="4" fill="none" strokeLinecap="round" />

                {showBoundingBoxes && characters.length > 1 && (
                    <g aria-label={`Character ${index + 1} bounding box`}>
                        <rect 
                            x={localBBox.minX} y={localBBox.minY}
                            width={localBBox.maxX - localBBox.minX} height={localBBox.maxY - localBBox.minY}
                            fill="rgba(96, 165, 250, 0.1)"
                            stroke="rgba(59, 130, 246, 0.8)"
                            strokeWidth={2 / charInstance.scale}
                            rx={10 / charInstance.scale}
                        />
                        <rect
                            x={localBBox.minX} y={localBBox.minY}
                            width={24 / charInstance.scale}
                            height={24 / charInstance.scale}
                            fill="rgba(59, 130, 246, 0.8)"
                            rx={8 / charInstance.scale}
                        />
                        <text
                            x={localBBox.minX + 12 / charInstance.scale}
                            y={localBBox.minY + 17 / charInstance.scale}
                            fill="white"
                            textAnchor="middle"
                            fontSize={16 / charInstance.scale}
                            fontWeight="bold"
